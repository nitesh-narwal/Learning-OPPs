...
public class Step5_AnonymousVsLambda {
    ...
    // NEW: Advanced comparison with streams
    public void advancedLambdaComparison() {
        
        System.out.println("\n" + "=".repeat(60));
        System.out.println("ADVANCED: LAMBDA IN STREAMS");
        System.out.println("=".repeat(60));
        
        java.util.List<Integer> numbers = java.util.Arrays.asList(10, 20, 30, 40, 50);
        
        System.out.println("\n🔴 ANONYMOUS CLASS APPROACH:");
        System.out.println("Using Iterator with anonymous class");
        
        java.util.Iterator<Integer> iterAnon = new java.util.Iterator<Integer>() {
            private int index = 0;
        ...
        int sumAnon = 0;
        while (iterAnon.hasNext()) {
            sumAnon += iterAnon.next();
        }
        System.out.println("Sum (Anonymous): " + sumAnon);
        
        System.out.println("\n🟢 LAMBDA APPROACH:");
        System.out.println("Using Stream with lambda");
        
        int sumLambda = numbers.stream()
                .filter(n -> n > 15)  // Lambda: filter
                .map(n -> n * 2)       // Lambda: map
                .reduce(0, Integer::sum);  // Lambda: reduce
        System.out.println("Sum (Lambda) with filter & map: " + sumLambda);
        System.out.println("\n✨ Lambda with Streams is 5x more concise!");
    }
    
    // NEW: Method references - even cleaner than lambda
    public void demonstrateMethodReferences() {
        
        System.out.println("\n" + "=".repeat(60));
        System.out.println("BONUS: METHOD REFERENCES (Cleaner than Lambda)");
        System.out.println("=".repeat(60));
        
        java.util.List<String> cars = java.util.Arrays.asList("BMW", "Audi", "Tesla", "Mercedes");
        ...
        System.out.println("Using System.out::println (even cleaner!):");
        cars.forEach(System.out::println);
        
        System.out.println("\n📌 SORTING:");
        java.util.List<Integer> nums = java.util.Arrays.asList(50, 20, 80, 10, 90);
        
        System.out.println("Using lambda comparator:");
        java.util.Collections.sort(nums, (a, b) -> a.compareTo(b));
        System.out.println(nums);
        
        nums = java.util.Arrays.asList(50, 20, 80, 10, 90);
        System.out.println("\nUsing method reference comparator:");
        java.util.Collections.sort(nums, Integer::compareTo);
        System.out.println(nums);
    }
    ...
    public void performanceComparison() {
        ...
        // Test 1: Anonymous class
        long start = System.nanoTime();
        int sum1 = 0;
        for (Integer num : largeList) {
            sum1 += new java.util.function.IntUnaryOperator() {
                @Override
        ...
        long timeAnon = System.nanoTime() - start;
        
        // Test 2: Lambda
        start = System.nanoTime();
        int sum2 = 0;
        for (Integer num : largeList) {
            java.util.function.IntUnaryOperator lambda = x -> x * 2;
            sum2 += lambda.applyAsInt(num);
        }
        long timeLambda = System.nanoTime() - start;
        
        System.out.println("\n📊 Processing 100,000 numbers:");
        System.out.println("Anonymous class time: " + (timeAnon / 1_000_000) + " ms");
        System.out.println("Lambda time: " + (timeLambda / 1_000_000) + " ms");
        System.out.println("Lambda is " + String.format("%.2f", (double) timeAnon / timeLambda) + "x faster");
    }
}
